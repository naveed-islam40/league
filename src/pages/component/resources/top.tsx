...
const Card: React.FC<CardProps> = ({
  ...
  <div className="bg-white dark:bg-black rounded-[7px] p-3 shadow-sm">
    <Image src={src} alt={title} className="md:h-[100px]" />
    <p className="nun font-normal text-[#B0B0B0] text-[12px] mt-4">{title}</p>
    <p className="nun font-medium text-[#121212] dark:text-white text-[12px] lg:text-[14px] mt-1">
      {description}
    </p>
...
export default function Top() {
  return (
    <>
      ...
      <div className="hidden md:grid md:grid-cols-3 xl:grid-cols-5 gap-5 mt-5">
        {data.map((item, index) => (
          <Card
            key={index}
            src={item.src}
